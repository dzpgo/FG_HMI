...
namespace UACSView.View_CraneMonitor
{
    ...
    /// 查询已经分配了的吊运指令
    /// </summary>
    public partial class Frm_02StatusCoil_Yard2Yard_Plan : FormBase
    {
        ...
        public Frm_02StatusCoil_Yard2Yard_Plan()
        {
            InitializeComponent();
        }
        #region 方法
        ...
        private void BinddataGridView1Data()
        {
            ...
            string sqlText = @"SELECT 0 AS CHECK_COLUMN,A.NEXT_UNIT_NO,A.COIL_NO,B.STOCK_NO,A.WEIGHT,A.OUTDIA,A.WIDTH,A.INDIA,A.FORBIDEN_FLAG,A.DUMMY_COIL_FLAG,A.L3_COIL_STATUS,A.PACK_CODE,CASE
            ...
            sqlText += " WHERE B.MAT_NO != 'NULL' AND B.STOCK_NO LIKE '%Z01%'";

            if (CoilNo == "" && SaddleNo == "")
            {
                sqlText += " AND A.L3_COIL_STATUS = '02' ";
            }
        ...
        private void BinddataGridView2Data()
        {
            ...
            try
            {
                string sqlText = @"SELECT 0 AS CHECK_COLUMN,A.STOCK_NO STOCK_NO , A.MAT_NO COIL_NO FROM YARD_TO_CLOSED_LIST A ";
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                foreach (DataGridViewRow r in dataGridView1.Rows)
                {
                    ...
                    {
                        StockNo = r.Cells["STOCK_NO"].Value.ToString();
                        PlanCoil = r.Cells["COIL_NO"].Value.ToString();
                        ...
                        //    }
                        //}
                        string sqlSelhalfcoil = string.Format(@"SELECT * FROM YARD_TO_CLOSED_LIST WHERE STOCK_NO = '{0}'", StockNo);
                        using (IDataReader rdr = DB2Connect.DBHelper.ExecuteReader(sqlSelhalfcoil))
                        {
                            if (!rdr.Read())
                            {
                                string sqlInshalfinsert = string.Format(@"INSERT INTO YARD_TO_CLOSED_LIST (STOCK_NO,MAT_NO) VALUES ('{0}','{1}')", StockNo, PlanCoil);
            ...
            if (MessageBox.Show("确定要删除勾选的计划？", "删除提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
            {
                try
                {
                    ...
                    foreach (DataGridViewRow r in dataGridView2.Rows)
                    {
                        ...
                        {
                            string planStockNO = r.Cells["STOCK_NO2"].Value.ToString();
                            string sql = string.Format("DELETE FROM YARD_TO_CLOSED_LIST WHERE STOCK_NO = '{0}'", planStockNO);
                            dt.Add(sql);
                        }
                    }

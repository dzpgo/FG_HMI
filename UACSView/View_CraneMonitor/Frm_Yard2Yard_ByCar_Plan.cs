...
namespace UACSView.View_CraneMonitor
{
    ...
    /// 查询已经分配了的吊运指令
    /// </summary>
    public partial class Frm_Yard2Yard_ByCar_Plan : FormBase
    {
        ...
        public Frm_Yard2Yard_ByCar_Plan()
        {
            InitializeComponent();
        }
        #region 方法
        ...
        private void BinddataGridView2Data()
        {
            ...
            try
            {
                string sqlText = @"SELECT 0 AS CHECK_COLUMN,A.STOCK_NO STOCK_NO , A.MAT_NO COIL_NO FROM YARD_TO_YARD_BY_CAR A ";
            ...
            try
            {
                foreach (DataGridViewRow r in dataGridView1.Rows)
                {
                    if (r.Cells["CHECK_COLUMN"].Value != null && Convert.ToInt32(r.Cells["CHECK_COLUMN"].Value) == 1)
                    {
                        ...
                        //    }
                        //}
                        string sqlSelhalfcoil = string.Format(@"SELECT * FROM YARD_TO_YARD_BY_CAR WHERE STOCK_NO = '{0}'", StockNo);
                        using (IDataReader rdr = DB2Connect.DBHelper.ExecuteReader(sqlSelhalfcoil))
                        {
                            if (!rdr.Read())
                            {
                                string sqlInshalfinsert = string.Format(@"INSERT INTO YARD_TO_YARD_BY_CAR (STOCK_NO,MAT_NO) VALUES ('{0}','{1}')", StockNo, PlanCoil);
                ...
                try
                {
                    ...
                    foreach (DataGridViewRow r in dataGridView2.Rows)
                    {
                        if (r.Cells["CHECK_COLUMN2"].Value != null && (Int32)r.Cells["CHECK_COLUMN2"].Value == 1)
                        {
                            ...
                            string sql = string.Format("DELETE FROM YARD_TO_YARD_BY_CAR WHERE STOCK_NO = '{0}'", planStockNO);
                            dt.Add(sql);
                        }
                    }

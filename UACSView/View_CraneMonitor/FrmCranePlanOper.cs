    ...
    public partial class FrmCranePlanOper : FormBase
    {
        ...
        private List<string> GetRules()
        {
            ...
            try
            {
                ...
                using (IDataReader rdr = DB2Connect.DB0Helper.ExecuteReader(sqlRule))
                {
      ...
      /*  static public object GetRuleItem(object ZHK_Item, object CPK_Item, object admin_Item)
        ...
        private void dataGridView1_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                sqlText += "WHERE HG_NO = '{0}' and CMD_STATUS = '{1}' and MAT_NO = '{2}' ORDER BY REC_TIME DESC";
                sqlText = string.Format(sqlText, hgNo, cmdStatus, matNo);
                dt_ack.Clear();
        ...
        private void GetCraneOrderOperData()
        {
            ...
            string craneNo = this.comboBox_craneNo.Text.Trim();
            //string craneNo = this.comboBox_craneNo.SelectedValue.ToString();        
            ...
            if (checkBox_ByRecTime.Checked)
            {
                ...
                // 时间跨度
                sqlText = @"SELECT 0 AS CHECK_COLUMN,UNIQUE_ID,ORDER_NO, CRANE_NO, MAT_NO, STOCK_NO, X, Y,
                            CASE
                            WHEN CMD_STATUS = 'S' THEN '起吊'
                            WHEN CMD_STATUS = 'E' THEN '落卷'
                            END AS CMD_STATUS,
                            CASE
                            WHEN CRANE_MODE = 1 THEN '遥控'
                            WHEN CRANE_MODE = 2 THEN '人工'
                            WHEN CRANE_MODE = 4 THEN '自动'
                            WHEN CRANE_MODE = 5 THEN '等待'
                            END AS CRANE_MODE,
                            (CASE ORDER_TYPE 
                            WHEN '11' THEN '运输链入库'
                            WHEN '13' THEN '卡车入库'                            
                            ...
                            DEL_FLAG, REC_TIME, HG_NO,SEND_FLAG FROM UACS_CRANE_ORDER_OPER A 
                           WHERE A.REC_TIME > '{0}' and A.REC_TIME < '{1}'";
                sqlText = string.Format(sqlText, recTime1, recTime2);
                if (craneNo == "全部"|| craneNo == "")
                {
                ...
                if (comboBox_orderType.Text.Trim() != "")
                {
                    if (comboBox_orderType.Text.Trim() == "入库")
                    {
                        string orderType = "(A.ORDER_TYPE = '11' OR A.ORDER_TYPE = '12' OR A.ORDER_TYPE = '13' OR A.ORDER_TYPE = '14' OR A.ORDER_TYPE = '15')";
                        sqlText = string.Format("{0} AND {1}", sqlText, orderType);
                        ...
                        //else
                        //{
                        //    sqlText = string.Format("{0} AND A.CRANE_NO LIKE '%{1}%' AND {2}", sqlText, craneNo, orderType);
                        //}
                    }
                    else if(comboBox_orderType.Text.Trim() == "出库")
                    {
                        string orderType = "(A.ORDER_TYPE = '22' OR A.ORDER_TYPE = '24' OR A.ORDER_TYPE = '28')";
                        sqlText = string.Format("{0} AND {1}", sqlText, orderType);
                    ...
                    {
                        string orderType = this.comboBox_orderType.SelectedValue.ToString();
                        ...
                        //else
                        //{
                        //    sqlText = string.Format("{0} AND A.CRANE_NO LIKE '%{1}%' AND A.ORDER_TYPE = '{2}'", sqlText, craneNo, orderType);
                        //}
                    }
                }
...
//                    FROM UACS_CRANE_ORDER_OPER A WHERE A.CRANE_NO LIKE '{0}'  ORDER BY A.UNIQUE_ID DESC  FETCH first {1} rows only";
                sqlText = @"SELECT 0 AS CHECK_COLUMN,UNIQUE_ID,ORDER_NO, CRANE_NO, MAT_NO, STOCK_NO, X, Y,
                            ...
                            DEL_FLAG, REC_TIME, HG_NO,SEND_FLAG FROM UACS_CRANE_ORDER_OPER A WHERE A.CRANE_NO LIKE '{0}'  ORDER BY A.UNIQUE_ID DESC  FETCH first {1} rows only";
        ...
        private void BindOrderType()
        {
            ...
            da.Columns.Add("TypeValue");
            da.Columns.Add("TypeName");
            ...
            //dr = da.NewRow();
            //dr["TypeValue"] = "07";
            //dr["TypeName"] = "台车→拆包/离线";
            //da.Rows.Add(dr);

            //dr = da.NewRow();
            //dr["TypeValue"] = "08";
            //dr["TypeName"] = "机组出口→机组出口";
            //da.Rows.Add(dr);

            //dr = da.NewRow();
            //dr["TypeValue"] = "0B";
            //dr["TypeName"] = "机组入口→拆包/离线";
            //da.Rows.Add(dr);

            //dr = da.NewRow();
            //dr["TypeValue"] = "26";
            //dr["TypeName"] = "拆包/离线→台车 库区→拆包/离线";
            //da.Rows.Add(dr);
            //绑定列表下拉框数据
            this.comboBox_orderType.DataSource = da;
            this.comboBox_orderType.DisplayMember = "TypeName";
            this.comboBox_orderType.ValueMember = "TypeValue";
        }
        ...
        private void btn_DGV_OUT_Click(object sender, EventArgs e)
        {
            try
            {

...
as Baosight.iSuperframe.Authorization.Interface.IAuthorization;
        ...
        public TruckStowageInT(bool _isOpenPasswordForm, string _bayNo, string parkNO)
        {
            InitializeComponent();
            this.Load += TruckStowageInT_Load;
        ...
        private void TruckStowageInT_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (parkingNO != "")//画面跳转
                {
                    cmbArea.Text = GetOperateArea(parkingNO);
        ...
        private void Inq_LoadMap(int thestowageId)
        {
            try
            {
                ...
                string sql = @"SELECT PARKING_STATUS FROM UACS_PARKING_STATUS WHERE PARKING_NO = '" + comb_ParkingNO.Text.Trim() + "'";
                using (IDataReader rdr = DB2Connect.DBHelper.ExecuteReader(sql))
                {
                    while (rdr.Read())
                    {
                        if (Convert.ToString(rdr["PARKING_STATUS"]).Trim() != "130")
                        {                            
                            return;
                        }
                ...
                string sqlText_LoadMap = @"SELECT  A.GROOVEID , A.MAT_NO, A.POS_ON_FRAME, A.X_CENTER, A.Y_CENTER, A.Z_CENTER,
                                    ...
                                    END as  ISUNLOAD , 
                                    CASE
                                        WHEN  A.MAT_DIRECTION = 0 THEN '不旋转'
                                        WHEN  A.MAT_DIRECTION = 1 THEN '旋转'
                                        ELSE '未知'
                                    END as  MAT_DIRECTION ,
                                    A.SLEEVE_LENGTH, A.PACKAGE_STATUS FROM UACS_TRUCK_STOWAGE_DETAIL A  ";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (IDataReader rdr = DB2Connect.DBHelper.ExecuteReader(sql))
                {
                    while (rdr.Read())
                    {
                        if (Convert.ToString(rdr["PARKING_STATUS"]).Trim() != "120")
                        {
                            MessageBox.Show("停车位状态非入库激光扫描完成！");
                            return;
                        }
                ...
                if (cancelFlag == false)
                {
                    ...
                    tagProvider_InStock.SetData("EV_NEW_PARKING_IN_PORTABLEEND", parkingNO);
                }
            }
            catch (Exception er)
            {
            ...
            try
            {
                ...
                if (cancelFlag == false)
                {
                   ...
                   // tagProvider_InStock.SetData("EV_NEW_PARKING_CARARRIVE", theOperator + "|" + date + "|" + shift + "|" + crew + "|" + parkingNO + "|" + carNO + "|" + isLoaded + "|" + headPostion + "|" + pullAbility + "|" + equno + "|" + cartype + "|" + parkingype);
                    ...
                    //更新UACS_PARKING_STATUS表，框架是否留用
                    string sql = string.Format("UPDATE UACS_PARKING_STATUS SET IS_STAY_CAR = {1} WHERE PARKING_NO = '{0}'", parkingNO, isKeep);
        ...
        private void GetComboxOnParking(ComboBox comBox)
        {
            ...
            try
            {
                ...
                string sqlText = @"SELECT DISTINCT ID as TypeValue,NAME as TypeName FROM UACS_YARDMAP_PARKINGSITE WHERE YARD_NO = '" + AreaNo + "'";
        ...
        //            str2 = "23";
        //        }
        //        else if (cmbArea.Text.Contains("Z01"))
        //        {
        //            str1 = "Z01";
        //            str2 = "01";
        //        }
        ...
        //    try
        //    {
        //        if (parkNO.Contains("Z21") && parkingNO.Contains("A"))
        //        {
        //            area = "6-1";
        //        }
        //        else if (parkNO.Contains("Z22") && parkingNO.Contains("B"))
        //        {
        //            area = "6-2";
        //        }
        //        else if (parkNO.Contains("C"))
        //        {
        //            area = "6-3通道";
        //        }
        ...
        private bool LoadLaserInfo(string strParkNo, int laserActionCount, string carHeardDrection, ParkLaserOut park)
        {
            ...
            try
            {
                ...
                {
                    park.DrawParksize(XStart, YEnd, XEnd - XStart, YEnd - YStart, carHeardDrection, "INC");
                }
                else
                {
                    park.DrawParksize(XStart, YEnd, XEnd - XStart, YEnd - YStart, carHeardDrection);
        ...
        private bool JumpToOtherForm(string currPark)
        {
            ...
            if (strStatus.Substring(0, 1) == "2" )
            {
                ...
                auth.OpenForm("02 车辆出库", true, cmbarea, currPark);
        ...
        private void btnReissue_Click(object sender, EventArgs e)
        {
            try
            {
                if (!(DialogResult.OK == MessageBox.Show("确定需要卸空补发吗？", "操作提示", MessageBoxButtons.OKCancel)))
                {
                    return;
                }
                string isLoaded = "";
                string sqlText = @"SELECT ISLOADED FROM UACS_PARKING_STATUS WHERE PARKING_NO = '" + comb_ParkingNO.Text.Trim() + "'";
                using (IDataReader rdr = DBHelper.ExecuteReader(sqlText))
                {
                    while (rdr.Read())
                    {
                ...
                if (comb_ParkingNO.Text.Trim() != "请选择" && comb_ParkingNO.Text.Trim() != "" && comb_ParkingNO.Text.Trim().Contains('Z') && txt_LoadMapID.Text.Trim() != "" && isLoaded == "1")
                {
                    string tagReissue = comb_ParkingNO.Text.Trim() + "|" + txt_LoadMapID.Text.Trim() + "|1|I|";
                    tagProvider_InStock.SetData("EV_NEW_PARKING_SEND_STOWAGE_TO_MES", tagReissue);
                    HMILogger.WriteLog(btnReissue.Text, "补发：" + comb_ParkingNO.Text.Trim(), LogLevel.Info, this.Text);
                    MessageBox.Show("卸空补发完成！");
                }
                else
                {
                    MessageBox.Show("该停车位未重车车到位或者未配载，不能补发");
                    return;
                }
            }
            catch (Exception er)

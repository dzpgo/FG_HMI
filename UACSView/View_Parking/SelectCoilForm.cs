...
namespace UACSParking
{
    ...
    public partial class SelectCoilForm : Form
    {
        ...
        private void SelectCoilForm_Load(object sender, EventArgs e)
        {
            ...
            //加载材料信息
            //BindMatStock(parkNO, txtGetPlanNo.Text.Trim(), txtGetMat.Text.Trim(), txtBoxStockNO.Text.Trim());
        ...
        /// 绑定材料位置信息
        /// </summary>
        private void BindMatStock(string packing, string planNo, string coilNo, string stockNo)
        {
            ...
            if (planNo != "" && planNo.Length > 3)
            {
                string toUpPlanNO = planNo.ToUpper().Trim();
                sqlText_All += " AND A.LOT_NO  like '" + "%" + toUpPlanNO + "%' ";
            }          
            if (coilNo != "" && coilNo.Length < 13)
            { 
                sqlText_All += " AND C.MAT_NO  like '" + "%" + coilNo + "%' ";
            }
            else if(coilNo != "" && coilNo.Length >= 13)
            {
                MessageBox.Show("卷号已超出12位，请重新输入！", "警告");
                return;
            }    
            
            string stock_No = "";
            if (stockNo != "" && !stockNo.Contains('-') && stockNo.Length < 11)
            {
                sqlText_All += " AND C.STOCK_NO  like '" + "%" + stockNo + "%' ";
            }
            else if (stockNo != "" && !stockNo.Contains('-') && stockNo.Length >= 11)
            {
                MessageBox.Show("库位号已超出10位，请重新输入！", "警告");
                return;
            }
            else if (stockNo != "" && stockNo.Contains('-'))
            {
                string[] stockNoArray = stockNo.Split('-');
                stock_No = String.Format("Z01{0}{1}1", stockNoArray[0], stockNoArray[1]);
                sqlText_All += " AND C.STOCK_NO  like '" + "%" + stock_No + "%' ";
            }
            else if(stockNo != "" && stockNo.Contains('-') && (stockNo.Length != 8 || stockNo.Length != 9))
            {
                MessageBox.Show("库位格式错误：请以“库-排-列”，如01-01，请重新输入！", "警告");
                return;
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //{
            //    MessageBox.Show(string.Format("输入库位：{0}格式不合法，请重新输入，格式为：库-排-列。", txtBoxStockNO.Text));
            //    txtBoxStockNO.Text = "";
            //    return;
            //}
            ...
            //    else
            //    {
            //        sqlText_All += "WHERE C.BAY_NO = '" + parkNO.Substring(0, 3) + "'";
            //    }
            //    if (carType == "200")
            //    {
        ...
        {
            if(parkingNo.Contains("Z61"))
            {
                bayNO = "Z61";
            }
            else if(parkingNo.Contains("Z73"))
            {
                bayNO = "Z73";
            }

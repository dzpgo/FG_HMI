...
namespace UACSView
{
    public partial class DuXin_Pack_Exit : FormBase
    {
        ...
        public DuXin_Pack_Exit()
        {
            ...
            this.Load += DuXin_Pack_Exit_Load;
        }
        ...
        void DuXin_Pack_Exit_Load(object sender, EventArgs e)
        {
            ...
            dicSaddleControls["PA-2WC0009"] = coilUnitSaddle9;
            ...
            coilUnitSaddleButton.MySaddleNo = "PA-2WC0001";
            ...
            Get_PA_Status();
            exitSaddleInfo.getExitSaddleDt(dgvExitSaddleInfo,constData.UnitNo_10);
            dgvRefresh();
            //是否开启定时器
        ...
        private void btn_PA_Status_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string value = "1";
                    inDatas["PA-2_IS_BUSY"] = value;
                    tagDataProvider.Write2Level1(inDatas, 1);
                    MessageBox.Show("已打开包装线繁忙状态！");
                    btn_PA_Status.Text = "包装线状态：繁忙";
        ...
        private void Get_PA_Status()
        {
            ...
            List<string> lstAdress = new List<string>();
            lstAdress.Clear();
            lstAdress.Add("PA-2_IS_BUSY");
            arrTagAdress = lstAdress.ToArray<string>();
            readTags();
            if (get_value("PA-2_IS_BUSY") == "0")
            {
                btn_PA_Status.Text = "包装线状态：正常";
                btn_PA_Status.BackColor = Color.LightGreen;
            }
            else if (get_value("PA-2_IS_BUSY") == "1")
            {
                btn_PA_Status.Text = "包装线状态：繁忙";
                btn_PA_Status.BackColor = Color.Yellow;
            }
        ...
        private void dgvExitSaddleInfo_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgvExitSaddleInfo.Columns[e.ColumnIndex].Name == "FORBIDEN_COIL_FLOW")
            {
                ...
                if (((DataGridViewButtonCell)dgvExitSaddleInfo.Rows[e.RowIndex].Cells["FORBIDEN_COIL_FLOW"]).UseColumnTextForButtonValue == true)
                {
                    ...
                    frmForbidenCoilFlow.UnitNo = constData.UnitNo_10;

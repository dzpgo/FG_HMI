...
namespace UACSView
{
    public partial class VIEW_YSL4ExitLineSaddle : FormBase
    {
        private Baosight.iSuperframe.Authorization.Interface.IAuthorization auth;
        public VIEW_YSL4ExitLineSaddle()
        {
            InitializeComponent();
            //反射获取双缓存
        ...
        bool dgvIsReFresh = true;
        private const string AREA_SAFE_EXIT_YSL4 = "AREA_SAFE_YSL4_EXIT";
        ...
        void VIEW_D108ExitLineSaddle_Load(object sender, EventArgs e)
        {
            ...
            //绑定鞍座控件
            //dicSaddleControls["YSL4WC0026"] = coilUnitSaddle26;
            //dicSaddleControls["YSL4WC0025"] = coilUnitSaddle25;
            //dicSaddleControls["YSL4WC0024"] = coilUnitSaddle24;
            //dicSaddleControls["YSL4WC0023"] = coilUnitSaddle23;
            dicSaddleControls["YSL4WC0010"] = coilUnitSaddle10;
            dicSaddleControls["YSL4WC0009"] = coilUnitSaddle9;
            dicSaddleControls["YSL4WC0008"] = coilUnitSaddle8;
            dicSaddleControls["YSL4WC0007"] = coilUnitSaddle7;
            dicSaddleControls["YSL4WC0006"] = coilUnitSaddle6;
            dicSaddleControls["YSL4WC0005"] = coilUnitSaddle5;
            dicSaddleControls["YSL4WC0004"] = coilUnitSaddle4;
            dicSaddleControls["YSL4WC0003"] = coilUnitSaddle3;
            dicSaddleControls["YSL4WC0002"] = coilUnitSaddle2;
            dicSaddleControls["YSL4WC0001"] = coilUnitSaddle1;

            //coilUnitStatus1.InitTagDataProvide(constData.tagServiceName);
            //coilUnitStatus1.MyStatusTagName = AREA_SAFE_EXIT_D102;

            //实例化机组鞍座处理类
            saddleMethod = new UnitSaddleMethod(constData.UnitNo_YSL4, constData.ExitSaddleDefine, constData.tagServiceName);
            saddleMethod.ReadDefintion();

            //绑定状态控件
            coilUnitStatus1.InitTagDataProvide(constData.tagServiceName);
            coilUnitStatus1.MyStatusTagName = "YSL4_EXIT_STEP_1_ENABLE";
            coilUnitStatus2.InitTagDataProvide(constData.tagServiceName);
            coilUnitStatus2.MyStatusTagName = "YSL4_EXIT_STEP_1_OP_UNLOCK";
            coilUnitStatus3.InitTagDataProvide(constData.tagServiceName);
            coilUnitStatus3.MyStatusTagName = "YSL4_EXIT_STEP_2_ENABLE";
            coilUnitStatus4.InitTagDataProvide(constData.tagServiceName);
            coilUnitStatus4.MyStatusTagName = "YSL4_EXIT_STEP_2_OP_UNLOCK";

            coilUnitStatus5.InitTagDataProvide(constData.tagServiceName);
            coilUnitStatus5.MyStatusTagName = AREA_SAFE_EXIT_YSL4;

            lineSaddleTag.InitTagDataProvider(constData.tagServiceName);

            coilUnitSaddleButton.MySaddleNo =  "YSL4WC0024";
            coilUnitSaddleButton1.MySaddleNo = "YSL4WC0015";
            coilUnitSaddleButton2.MySaddleNo = "YSL4WC0016";
            ...
            exitSaddleInfo.getExitSaddleDt(dgvExitSaddleInfo, constData.UnitNo_YSL4);
            dgvRefresh();
            //是否开启定时器
        ...
        private void dgvExitSaddleInfo_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgvExitSaddleInfo.Columns[e.ColumnIndex].Name == "FORBIDEN_COIL_FLOW")
            {
                ...
                if (((DataGridViewButtonCell)dgvExitSaddleInfo.Rows[e.RowIndex].Cells["FORBIDEN_COIL_FLOW"]).UseColumnTextForButtonValue == true)
                {
                    ...
                    frmForbidenCoilFlow.UnitNo = constData.UnitNo_1;
        ...
        private void dgvExitSaddleInfo_MouseEnter(object sender, EventArgs e)
        {
            dgvIsReFresh = false;
        }

        private void dgvExitSaddleInfo_MouseLeave(object sender, EventArgs e)
        {
            dgvIsReFresh = true;
        }
        ...
        private void executeNonQuery(int i)
        {
            string sql = @"UPDATE UACS_LINE_EXIT_DIFFERENTIATE_UNIT  SET IS_DIFFERENTIATE_UNIT = {0} WHERE UNIT_NO = '{1}'";
            sql = string.Format(sql, i, constData.UnitNo_YSL4);
            try
            {
                DB2Connect.DBHelper.ExecuteNonQuery(sql);
            }
        ...
        private void GetUnitFlow()
        {
            int IS_DIFFERENTIATE_UNIT = 0;
            string sql = @"SELECT IS_DIFFERENTIATE_UNIT FROM UACS_LINE_EXIT_DIFFERENTIATE_UNIT WHERE UNIT_NO = '{0}'";
            sql = string.Format(sql, constData.UnitNo_YSL4);
            try
            {
                using (IDataReader rdr = DB2Connect.DBHelper.ExecuteReader(sql))
                {
                    while (rdr.Read())
                    {
                        if (rdr["IS_DIFFERENTIATE_UNIT"] != DBNull.Value)
                        {
                            IS_DIFFERENTIATE_UNIT = Convert.ToInt32(rdr["IS_DIFFERENTIATE_UNIT"]);
                        }
                    }
                }
                if(IS_DIFFERENTIATE_UNIT == 1)
                {
                    btnD302Flow_No.Enabled = true;
                    btnD302Flow_Yes.Enabled = false;
                    btnD302Flow_No.BackColor = Color.Gray;
                    btnD302Flow_Yes.BackColor = Color.Green;
                }
                else
                {
                    btnD302Flow_No.Enabled = false;
                    btnD302Flow_Yes.Enabled = true;
                    btnD302Flow_No.BackColor = Color.Green;
                    btnD302Flow_Yes.BackColor = Color.Gray;
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnYSL4Flow_No_Click(object sender, EventArgs e)
        {
            executeNonQuery(0);
            //GetUnitFlow();
        }

        private void btnYSL4Flow_Yes_Click(object sender, EventArgs e)
        {
            executeNonQuery(1);
            //GetUnitFlow();
        }

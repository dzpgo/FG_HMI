...
namespace UACSView
{
    public partial class VIEW_D374EntryLineSaddle : FormBase
    {
        ...
        public VIEW_D374EntryLineSaddle()
        {
            ...
            this.Load += VIEW_D174EntryLineSaddle_Load;
        ...
        private const string AREA_SAFE_D374_ENTRY = "AREA_SAFE_D374_WR";
        ...
        void VIEW_D174EntryLineSaddle_Load(object sender, EventArgs e)
        {
            auth = FrameContext.Instance.GetPlugin<IAuthorization>() as IAuthorization;
            #region 绑定鞍座控件
            //dicSaddleControls["D208WR1A01"] = coilUnitSaddle1;
            dicSaddleControls["D374WR0003"] = coilUnitSaddle2;
            dicSaddleControls["D374WR0002"] = coilUnitSaddle3;
            dicSaddleControls["D374WR0001"] = coilUnitSaddle4;
            //dicSaddleControls["D208WR1A05"] = coilUnitSaddle5;
            //dicSaddleControls["D208WR1A06"] = coilUnitSaddle6;           
            #endregion

            //实例化机组鞍座处理类
            saddleMethod = new UnitSaddleMethod(constData.UnitNo_D508, constData.EntrySaddleDefine, constData.tagServiceName);
            saddleMethod.ReadDefintion();

            coilUnitStatus1.InitTagDataProvide(constData.tagServiceName);
            coilUnitStatus1.MyStatusTagName = AREA_SAFE_D374_ENTRY;
            coilUnitStatus2.InitTagDataProvide(constData.tagServiceName);
            coilUnitStatus2.MyStatusTagName = "D374_ENTRY_STEP_ENABLE";
            coilUnitStatus3.InitTagDataProvide(constData.tagServiceName);
            coilUnitStatus3.MyStatusTagName = "D374_ENTRY_STEP_1_OP_UNLOCK";

            lineSaddleTag.InitTagDataProvider(constData.tagServiceName);

            coilUnitSaddleButton1.MySaddleNo = "D374WR0001";
            coilUnitSaddleButton2.MySaddleNo = "D374WR0002";
            coilUnitSaddleButton3.MySaddleNo = "D374WR0003";
            ...
            conEntrySpecAction1.UnitNo = constData.UnitNo_D374;
            conEntrySpecAction1.BayNo = "Z73";
            ...
            entrySaddleInfo.getEntrySaddleDt(dataGridViewSaddleMessage, constData.UnitNo_D374);
            CoilPlan();
            //dgvColor();
            //是否开启定时器
            timer1.Enabled = true;
        ...
        {
            entrySaddleInfo.getL2PlanByUnitNo(dataGridViewPlanNum, constData.UnitNo_D374);
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                coilEntryMode1.UnitNO = constData.UnitNo_D374;
        ...
        private void timer3_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                saddleMethod.GetCoilLabel(constData.UnitNo_D374, lblNextCoil, label2);
        ...
        public static void ClearMemory()
        {
            ...
            {
                VIEW_D508EntryLineSaddle.SetProcessWorkingSetSize(System.Diagnostics.Process.GetCurrentProcess().Handle, -1, -1);
            }
        }
        #endregion
        ...
        private void btnExecuteNonQuery(int MANU_VALUE, DataGridView dgv)
        {
            try
            {
                ...
                if (MANU_VALUE == 1)
                {
                    HMILogger.WriteLog("带头旋转", "D508机组入口,钢卷" + strCoil + "设置为旋转180°", LogLevel.Info, this.Text);
                }
                else if (MANU_VALUE == 0)
                {
                    HMILogger.WriteLog("带头旋转", "D508机组入口,钢卷" + strCoil + "设置为不旋转", LogLevel.Info, this.Text);
                }
            }
            catch (Exception ex)
            {

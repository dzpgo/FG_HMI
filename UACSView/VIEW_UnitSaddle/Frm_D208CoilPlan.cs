...
namespace UACSView.VIEW_UnitSaddle
{
    ...
    /// 查询已经分配了的吊运指令
    /// </summary>
    public partial class Frm_D208CoilPlan : FormBase
    {
        ...
        private void BinddataGridView1Data()
        {
            ...
            string Row = txtRow.Text.ToString().Trim();
            string Col = txtCol.Text.ToString().Trim();
            ...
            if (Row != "")
            {                              
                    sqlText += " AND B.STOCK_NO LIKE '%Z07" + Row.PadLeft(2,'0') + "%'";                                          
            }

            if (Col != "")
            {
                sqlText += " AND B.STOCK_NO LIKE '%" + Col.PadLeft(2,'0') + "1%'";
            }
        ...
        private void RefleshPlanOrder()
        {
            ...
            {
                string sqlSelhalfnextcoil = string.Format(@"UPDATE UACS_LINE_NEXTCOIL SET NEXT_COILNO = '' WHERE UNIT_NO = 'D208_Z22'");
            ...
            else
            {
                for (int i = 0; i < dataGridView2.Rows.Count; i++)
                {
                    ...
                    int plan_SEQ = i;
                    string sql = string.Format("UPDATE UACS_LINE_L2PLAN SET PLAN_SEQ = {1} WHERE COIL_NO = '{0}'", coil_NO, plan_SEQ);
                    DBHelper.ExecuteNonQuery(sql);

                    if (i == 0)
                    {
                        string sqlSelhalfnextcoil = string.Format(@"UPDATE UACS_LINE_NEXTCOIL SET NEXT_COILNO = '{0}' WHERE UNIT_NO = 'D208_Z22'", coil_NO);
                        DBHelper.ExecuteNonQuery(sqlSelhalfnextcoil);
                    }
                }
            }

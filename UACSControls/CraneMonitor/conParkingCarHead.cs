...
{
    public partial class conParkingCarHead : UserControl
    {
        public conParkingCarHead()
        {
            InitializeComponent();
            this.SetStyle(
        ...
        public void refreshControl(ParkingBase theParkingBase, long baySpaceX, long baySpaceY, int panelWidth, int panelHeight, bool xAxisRight, bool yAxisDown)
        {
            try
            {
                ...
                {
                    location_X = (baySpaceX - Convert.ToDouble(theParkingBase.X_END)) * xScale;
                }
                ...
                if (yAxisDown == true)
                {
                    ...
                    if(theParkingBase.HeadPostion == "S")
                    {
                        location_Y = Convert.ToDouble(theParkingBase.Y_END) * yScale;
                    }
                    else
                    {
                        location_Y = (Convert.ToDouble(theParkingBase.Y_START) - 1500) * yScale;
                    }
                }
                else
                {
                    ...
                    if (theParkingBase.HeadPostion == "S")
                    {
                        location_Y = Convert.ToDouble(baySpaceY - theParkingBase.Y_START) * yScale;
                    }
                    else
                    {
                        location_Y = (Convert.ToDouble(baySpaceY - theParkingBase.Y_END) - 1500) * yScale;
                    }
                }

                //修改鞍座控件的宽度和高度
                this.Width = Convert.ToInt32(theParkingBase.CarWidth * xScale);
                this.Height = Convert.ToInt32((1500) * yScale);

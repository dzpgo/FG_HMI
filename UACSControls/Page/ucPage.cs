...
namespace UACSControls.Page
{
    public partial class ucPage : UserControl
    {
        private string _cfgPageSize = "30,50,100,200"; //ConfigurationManager.AppSettings["ucPageSize"];
        ...
        void txtJumpPage_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == 13)
            {
                if (null != this.JumpPageEvent && !string.IsNullOrEmpty(this.txtJumpPage.Text) && !this.txtJumpPage.Text.Equals("页码"))
                {
        ...
        private void txtJumpPage_Enter(object sender, EventArgs e)
        {
            if (textboxHasText == false)
                txtJumpPage.Text = "";

            txtJumpPage.ForeColor = Color.Black;
        }

        private void txtJumpPage_Leave(object sender, EventArgs e)
        {
            txtJumpPageLeave();
        }
        private void txtJumpPageLeave()
        {
            if (string.IsNullOrEmpty(txtJumpPage.Text))
            {
                txtJumpPage.Text = "页码";
                txtJumpPage.ForeColor = Color.LightGray;

    ...
    public partial class CoilPicture : UserControl
    {
     ...
     /// </summary>
        ...
        private void contextMenuStrip1_Opening(object sender, CancelEventArgs e)
        {
            ...
            {
                冷卷吊离ToolStripMenuItem.Visible = true;
                热卷吊入ToolStripMenuItem.Visible = true;
                初始化水槽鞍座状态ToolStripMenuItem1.Visible = true;
            }
        ...
        private void 热卷确认吊入ToolStripMenuItem2_Click(object sender, EventArgs e)
        {
            ...
            string saddleNO = this.PosName.Substring(0, 8);
            string sql = @"SELECT A.STOCK_STATUS,A.LOCK_FLAG,B.CONFIRM_FLAG,B.AREA_ID FROM UACS_WATER_TANK_AREA_STOCK_DEFINE B LEFT JOIN UACS_YARDMAP_STOCK_DEFINE A  ON B.STOCK_NO = A.STOCK_NO WHERE A.STOCK_NO = '" + saddleNO + "'";
            using (IDataReader rdr = DBHelper.ExecuteReader(sql))
            {
                while (rdr.Read())
                {
        ...
        private void 热卷删除确认ToolStripMenuItem4_Click(object sender, EventArgs e)
        {
            string saddleNO = this.PosName.Substring(0, 8);
            string sql = @"SELECT CONFIRM_FLAG FROM UACS_WATER_TANK_AREA_STOCK_DEFINE WHERE STOCK_NO = '" + saddleNO + "'";
            using (IDataReader rdr = DBHelper.ExecuteReader(sql))
            {
                while (rdr.Read())
                {
            ...
            string sql2 = @"UPDATE UACS_WATER_TANK_AREA_STOCK_DEFINE SET CONFIRM_FLAG = 0, COIL_NO = NULL WHERE STOCK_NO = '" + saddleNO + "' AND CONFIRM_FLAG = 10";
            DBHelper.ExecuteNonQuery(sql2);
            MessageBox.Show("删除确认成功！");
            HMILogger.WriteLog("删除吊入", "鞍座号：" + saddleNO, LogLevel.Info, this.Text);
        }

        private void 冷卷确认吊离ToolStripMenuItem2_Click(object sender, EventArgs e)
        {
        ...
        private void 冷卷删除确认ToolStripMenuItem5_Click(object sender, EventArgs e)
        {

            string saddleNO = this.PosName.Substring(0,8);
            string sql1 = @"SELECT CONFIRM_FLAG FROM UACS_WATER_TANK_AREA_STOCK_DEFINE WHERE STOCK_NO = '" + saddleNO + "'";
            using (IDataReader rdr = DBHelper.ExecuteReader(sql1))
            {
                while (rdr.Read())
                {
            ...
            string sql = @"UPDATE UACS_WATER_TANK_AREA_STOCK_DEFINE SET CONFIRM_FLAG = 20 WHERE STOCK_NO = '" + saddleNO + "' AND CONFIRM_FLAG = 30";
            DBHelper.ExecuteNonQuery(sql);
            MessageBox.Show("删除确认成功！");
            HMILogger.WriteLog("删除吊离", "鞍座号：" + saddleNO, LogLevel.Info, this.Text);
        }

        private void 初始化水槽鞍座状态ToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            string saddleNO = this.PosName.Substring(0,8);
            DialogResult ret = MessageBox.Show("是否确定要清除鞍座的占位信息？", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
            if (ret == DialogResult.Cancel)
            ...
            else
            {
                ...
                if (retValue != DialogResult.Cancel)
                {
                    if (retValue == DialogResult.OK)
                    {
                        string sql = @"UPDATE UACS_WATER_TANK_AREA_STOCK_DEFINE SET CONFIRM_FLAG = 0,COIL_NO = NULL WHERE STOCK_NO = '" + saddleNO + "'";
                        DBHelper.ExecuteNonQuery(sql);
                        string sql2 = @"UPDATE UACS_YARDMAP_STOCK_DEFINE SET STOCK_STATUS = 0,LOCK_FLAG = 0,MAT_NO = NULL WHERE STOCK_NO = '" + saddleNO + "'";
                        DBHelper.ExecuteNonQuery(sql2);
                        MessageBox.Show("已成功初始化鞍座！");

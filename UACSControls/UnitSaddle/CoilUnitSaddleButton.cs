    ...
    public partial class CoilUnitSaddleButton : UserControl
    {
        ...
        private void btnLock_Click(object sender, EventArgs e)
        {
            ...
            if (btnLock.BackColor == Color.White)
            {
                ...
                if(mySaddleNo.Contains("PA"))
                {
                    DialogResult dr = MessageBox.Show("确定要进行对步进梁锁定操作吗？", "操作提示", btn);
        ...
        private void btnUnlock_Click(object sender, EventArgs e)
        {
            ...
            if (btnUnlock.BackColor == Color.White)
            {
                ...
                if(mySaddleNo.Contains("PA"))
                {
                    DialogResult dr = MessageBox.Show("确定要进行对步进梁解锁操作吗？", "操作提示", btn);
                    if (dr == DialogResult.OK)
                    {
                        if (lineSaddleTag != null && mySaddleTagName != "")
                        {
                            lineSaddleTag.setTagValue(this.mySaddleTagName, 0, out error);
                            ParkClassLibrary.HMILogger.WriteLog("机组鞍座解锁", "解锁镀锌包装线步进梁", ParkClassLibrary.LogLevel.Info, "机组鞍座");
                        }
                        if (error != string.Empty)
                        {
                            MessageBox.Show(error);

    ...
    public partial class ConTrolleyTag : UserControl
    {
        ...
        public void SetReadyA_B()
        {
            try
            {
                ...
                lstAdress.Add("MC_Z62_Z63_FORWARD");         //动车信号
                lstAdress.Add("MC_Z63_Z62_FORWARD");         //动车信号
                lstAdress.Add("MC_Z62_Z63_BACK");            //动车信号
                lstAdress.Add("MC_Z63_Z62_BACK");            //动车信号
        ...
        {
            if (btnMove.Text == "去Z62")
            {             
                    MessageBoxButtons btn = MessageBoxButtons.OKCancel;
                    DialogResult dr = MessageBox.Show("确定要过跨台车去Z62跨吗？", "操作提示", btn);
                    if (dr == DialogResult.OK)
                    {
                        inDatas["MC_Z62_FORWARD"] = 1;
            ...
            else if (btnMove.Text == "去Z63")
            {             
                   MessageBoxButtons btn = MessageBoxButtons.OKCancel;
                   DialogResult dr = MessageBox.Show("确定要过跨台车去Z63跨吗？", "操作提示", btn);
                   if (dr == DialogResult.OK)
                   {
                       inDatas["MC_Z63_FORWARD"] = 1;
                       tagDataProvider.Write2Level1(inDatas, 1);
                       ParkClassLibrary.HMILogger.WriteLog(btnMove.Text, "过跨台车" + btnMove.Text, ParkClassLibrary.LogLevel.Info, "过跨台车画面");
                   }
            }        
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            if(btnMove.Text == "去Z62")
            {
                inDatas["MC_Z62_FORWARD"] = 0;
                tagDataProvider.Write2Level1(inDatas, 1);
            }
            else if(btnMove.Text == "去Z63")
            {
                inDatas["MC_Z63_FORWARD"] = 0;
                tagDataProvider.Write2Level1(inDatas, 1);
            }
            else
            {

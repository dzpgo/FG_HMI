    ...
    public partial class InventoryShow : FormBase
    {
        ...
        private static Baosight.iSuperframe.Common.IDBHelper DBHelper
        {
            get
            {
                if (dbHelper == null)
                {
                    try
                    {
                        dbHelper = Baosight.iSuperframe.Common.DataBase.DBFactory.GetHelper("2030YLK");//平台连接数据库的Text
                    }
       ...
       /// <param name="id"></param>
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < dgvDetail.RowCount; i++)
                {
                    ...
                    ////成品卷与中间料区分入库
                    //string BAY_NO = STOCK_NO.Substring(0, 3);//跨别
                    //int ROW_NO = Convert.ToInt32(STOCK_NO.Substring(3, 2));//行号
                    //string ROW_COL_NO = STOCK_NO.Substring(3);//排列
                    //int COIIL_STATUS = 0;//卷状态
                    //string sql = @"SELECT L3_COIL_STATUS FROM UACS_YARDMAP_COIL WHERE COIL_NO = '" + MAT_NO + "'";
                    //using (IDataReader rdr = DB2Connect.DBHelper.ExecuteReader(sql))
                    //{
                    //    while (rdr.Read())
                    //    {
                    //        COIIL_STATUS = Convert.ToInt32(rdr["L3_COIL_STATUS"]);
                    //    }
                    //}
                    //if(COIIL_STATUS < 20)
                    //{
                    //    if (BAY_NO == "Z21")
                    //    {
                    //        if (ROW_NO > 90 && ROW_NO < 97)
                    //        {
                    //            STOCK_NO = "Z08" + ROW_COL_NO;
                    //        }
                    //    }
                    //    else if(BAY_NO == "Z23")
                    //    {
                    //        if (ROW_NO > 0 && ROW_NO < 15)
                    //        {
                    //            STOCK_NO = "Z06" + ROW_COL_NO;
                    //        }
                    //    }
                    //}
        ...
        private void BindingAllCheckCtrlForDgv(DataGridView dgv, CheckBox allCheckBox)
        {
            ...
            int x = dgv.Columns[0].Width / 2 - 22;
            int y = dgv.ColumnHeadersHeight / 2 - 10 ;
        ...
        private void btn_ForceConfirm_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (cbbId.Text.Substring(0, 3) == "Z01")
        ...
        private void dgvCheck_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1 && e.ColumnIndex == 0)
            {
                ...
                //    dgvCheck.Rows[e.RowIndex].Cells[e.ColumnIndex].EditedFormattedValue.ToString() == "False")
                //{
                //    dgvCheck.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = true;
                //}
                //else

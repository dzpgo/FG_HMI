        ...
        public void getExitSaddleDt(DataGridView _dgv, string _unitNo)
        {
            ...
            try
            {
                string sqlText = @"SELECT A.SADDLE_L2NAME,A.STOCK_NO,B.COIL_NO,C.WEIGHT,C.ACT_WEIGHT,C.WIDTH,C.ACT_WIDTH,C.INDIA,C.OUTDIA,C.L3_COIL_STATUS,
                                    CASE
                                        WHEN  C.COIL_OPEN_DIRECTION = 0 THEN '正'
                                        WHEN  C.COIL_OPEN_DIRECTION = 1 THEN '反' 
                                        WHEN  C.COIL_OPEN_DIRECTION = 2 THEN '待判'                                         
                                    END as  COIL_OPEN_DIRECTION , 
                                    CASE
                                        WHEN  C.PACK_FLAG = 1 THEN '已包装'
                                        WHEN  C.PACK_FLAG = 0 THEN '未包装'
                                    END as  PACK_FLAG , ";
                sqlText += "C.NEXT_UNIT_NO,C.PACK_CODE ";
                sqlText += ",B.WORK_ORDER_NO,C.PRODUCT_TIME ";
            ...
            if (hasSetColumn == false)
            {
                    ...
                    dt.Columns.Add("PRODUCT_TIME", typeof(String));
